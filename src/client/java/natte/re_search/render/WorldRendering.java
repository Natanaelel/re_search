...
public class WorldRendering {
    ...
    private static void rotateBasedOnPosition(WorldRenderContext context, MinecraftClient client) {
        if (getMarkedInventoriesSize() == 0)
            return;
        if (client.world.getTime() > HighlightRenderer.renderStartTimestamp + Config.autoHideTime * 20)
            return;
        ...
        for (MarkedInventory inventory : getMarkedInventories()) {
            ...
            // float h = MathHelper.lerp((float)tickDelta,
            // (float)playerEntity.prevStrideDistance, (float)playerEntity.strideDistance);
            // // matrixStack.translate(-MathHelper.sin((float)(g * (float)Math.PI)) * h *
            // 0.5f, Math.abs(MathHelper.cos((float)(g * (float)Math.PI)) * h), 0.0f);
            // //
            // matrixStack.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(MathHelper.sin((float)(g
            // * (float)Math.PI)) * h * 3.0f).invert().scale(scale2));
            // //
            // matrixStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(Math.abs(MathHelper.cos((float)(g
            // * (float)Math.PI - 0.2f)) * h) * 5.0f).invert().scale(scale2));

            // /* ^ combat view bobbing ^ */

            Vec3d relativeBlockPosition = new Vec3d(blockPos.getX(), blockPos.getY(), blockPos.getZ())
            ...
            matrixStack.push();
            {
                ...
                // //
                // matrixStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(Math.abs(MathHelper.cos((float)(g
                // * (float)Math.PI - 0.2f)) * h) * 5.0f).scale(scale2));
                // //
                // matrixStack.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(MathHelper.sin((float)(g
                // * (float)Math.PI)) * h * 3.0f).scale(scale2));
                // // matrixStack.translate(MathHelper.sin((float)(g * (float)Math.PI)) * h *
                // 0.5f * -scale2, -Math.abs(MathHelper.cos((float)(g * (float)Math.PI)) * h) *
                // -scale2, 0.0f);
                // }
                ...
                matrixStack.multiply(RotationAxis.POSITIVE_Y
                        .rotation((float) MathHelper.atan2(-transformedPosition.x, -transformedPosition.z)));
                matrixStack.multiply(RotationAxis.POSITIVE_X.rotation((float) MathHelper.atan2(transformedPosition.y,
                        Math.hypot(transformedPosition.x, transformedPosition.z))));

                // float smoothing = 100f;
                // smoothing = client.player.headYaw;
                // scale = (float)Math.log(Math.exp(scale * smoothing)+Math.exp(0.02f *
                // smoothing)) / smoothing;
                // scale = (float)Math.log(Math.exp(scale * smoothing)+Math.exp(((scale - 0.02f)
                // * 0.5f + 0.02f) * smoothing)) / smoothing;
                // if(scale > 0.1)
                // largest 0.13
                // medium 0.02
                ...
                for (ItemStack itemStack : inventory.inventory) {
                    ...
                    itemRenderer.renderItem(itemStack, ModelTransformationMode.GUI, 0xff, OverlayTexture.DEFAULT_UV,
                            matrixStack, vertexConsumers, client.world, 0);
                    itemRenderer.renderItem(null, itemStack, ModelTransformationMode.GUI, false, matrixStack,
                            vertexConsumers, client.world, 0xff, OverlayTexture.DEFAULT_UV, 0);
            ...
            matrixStack.push();
            {
                ...
                for (ItemStack itemStack : inventory.containers) {
                    ...
                    matrixStack.translate(x * 1.125f - inventory.containers.size() / 2f + 0.5f
     ...
     * Matrix4f positionMatrix = matrixStack.peek().getPositionMatrix();
     * 
     * Vector3f fpos = new Vector3f((float) pos.getX(), (float) pos.getY(), (float)
     * pos.getZ());
     * 
     ...
     * float v0 = 0.0f;
     * float v1 = 1.0f;
     * buffer.vertex(positionMatrix, v0, v0, v0).color(0xffffffff).texture(1f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v0, v1, v0).color(0xffffffff).texture(1f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v1, v1, v0).color(0xffffffff).texture(0f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v1, v0, v0).color(0xffffffff).texture(0f,
     * 1f).next();
     * 
     * buffer.vertex(positionMatrix, v0, v1, v1).color(0xffffffff).texture(0f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v0, v0, v1).color(0xffffffff).texture(0f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v0, v1).color(0xffffffff).texture(1f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v1, v1).color(0xffffffff).texture(1f,
     * 0f).next();
     * 
     * buffer.vertex(positionMatrix, v0, v0, v0).color(0xffffffff).texture(0f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v1, v0, v0).color(0xffffffff).texture(0f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v0, v1).color(0xffffffff).texture(1f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v0, v0, v1).color(0xffffffff).texture(1f,
     * 0f).next();
     * 
     * buffer.vertex(positionMatrix, v1, v1, v0).color(0xffffffff).texture(0f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v0, v1, v0).color(0xffffffff).texture(0f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v0, v1, v1).color(0xffffffff).texture(1f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v1, v1).color(0xffffffff).texture(1f,
     * 0f).next();
     * 
     ...
     * 0f).next();
     * 
     * buffer.vertex(positionMatrix, v1, v0, v1).color(0xffffffff).texture(0f,
     * 0f).next();
     * buffer.vertex(positionMatrix, v1, v0, v0).color(0xffffffff).texture(0f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v1, v0).color(0xffffffff).texture(1f,
     * 1f).next();
     * buffer.vertex(positionMatrix, v1, v1, v1).color(0xffffffff).texture(1f,
     * 0f).next();
     * 
